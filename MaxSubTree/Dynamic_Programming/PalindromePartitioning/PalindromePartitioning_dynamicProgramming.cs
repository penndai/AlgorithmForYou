...
namespace Algorithm_Multiple.PalindromePartitioning
{   
    public class PalindromePartitioning_dynamicProgramming
    {
        ...
        private static int minPalPartion(String str)
        {
            ...
            for (L = 2; L <= n; L++)
            {
                ...
                for (i = 0; i < n - L + 1; i++)
                {
                    ...
                    if (L == 2)
                        P[i, j] = (str[i] == str[j]);
                    else // abccba -> check first a == last a, and bccb is palindrome
                    ...
                    {
                        //by using cut any possibility to decide whether the string is palindrome
